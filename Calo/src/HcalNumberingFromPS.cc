...
HcalNumberingFromPS::HcalNumberingFromPS(const edm::ParameterSet& conf) {
  ...
  for (auto & phi : phibin_) phi *= deg;
  for (auto & phi : phioff_) phi *= deg;
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: EtaTable with " 
			      << etaTable_.size() << " elements";
  for (unsigned k=0; k<etaTable_.size(); ++k)
    edm::LogVerbatim("HcalSim") << "EtaTable[" << k << "] = " << etaTable_[k];
  edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: PhiBin with " 
			      << phibin_.size() << " elements";
  for (unsigned k=0; k<phibin_.size(); ++k)
    edm::LogVerbatim("HcalSim") << "PhiBin[" << k << "] = " << phibin_[k];
  edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: PhiOff with " 
			      << phioff_.size() << " elements";
  for (unsigned k=0; k<phioff_.size(); ++k)
    edm::LogVerbatim("HcalSim") << "PhiOff[" << k << "] = " << phioff_[k];
  edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: EtaMin/EtaMax with " 
			      << etaMin_.size() << ":" << etaMax_.size() 
			      << " elements";
  for (unsigned k=0; k<etaMin_.size(); ++k)
    edm::LogVerbatim("HcalSim") << "EtaMin[" << k << "] = " << etaMin_[k]
				<< " EtaMax[" << k << "] = " << etaMax_[k];
  edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: EtaHBHE " << etaHBHE_
			      << " DepthHBHE " << depthHBHE_[0] << ":"
			      << depthHBHE_[1] << " RMinHO "
			      << rMinHO_ << " zHO with " << zHO_.size()
			      << " elements";
...
#ifdef EDM_ML_DEBUG
    edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: Ring " << ring+1
				<< " with " << segmentation_[ring].size()
				<< " layers";
    for (unsigned int k=0; k<segmentation_[ring].size(); ++k)
      edm::LogVerbatim("HcalSim") << "Layer[" << k << "] = " 
				  << segmentation_[ring][k];
#endif
  }
}

HcalNumberingFromDDD::HcalID HcalNumberingFromPS::unitID(int det, int layer, 
							 int depth,
							 const math::XYZVectorD& pos) const {

  int                subdet = ((det == 3) ? static_cast<int>(HcalBarrel) :
			       static_cast<int>(HcalEndcap));
  ...
  if (deteta.first == static_cast<int>(HcalBarrel)) {
     ...
     if ((deteta.second == etaHBHE_) && 
	 (newDepth > depthHBHE_[0])) newDepth = depthHBHE_[0];
  } else if (deteta.first == static_cast<int>(HcalEndcap)) {
    newDepth = segmentation_[deteta.second-1][layer-1];
    if ((deteta.second == etaHBHE_) && 
	(newDepth < depthHBHE_[1])) newDepth = depthHBHE_[1];
...
#ifdef EDM_ML_DEBUG
  edm::LogVerbatim("HcalSim") << "HcalNumberingFromPS:: det " << det 
			      << ":" << subdet << ":" << deteta.first 
			      << "\t Eta " << pos.Eta() << ":" << deteta.second
			      << "\t Phi " << pos.Phi() << ":" << iphi.first
			      << ":" << iphi.second << "\t Layer|Depth " 
			      << layer << ":" << depth << ":" << newDepth;
#endif
  return HcalNumberingFromDDD::HcalID(deteta.first, zside, newDepth, 
				      deteta.second, iphi.first, iphi.second,
				      layer);
}
...
std::pair<int,int> HcalNumberingFromPS::getEta(const int& det, 
  ...
  if (pos.Rho() > rMinHO_) {
    ...
    if (z > zHO_[3]) {
      if (eta <= etaTable_[10]) eta = etaTable_[10]+0.001;
    } else if (z > zHO_[1]) {
      if (eta <= etaTable_[4])  eta = etaTable_[4]+0.001;
    }
  }
  for (unsigned int i = 1; i < etaTable_.size(); i++) {
    if (eta < etaTable_[i]) { ieta = i; break; }
  }

  if ((subdet == static_cast<int>(HcalBarrel)) || 
      (subdet == static_cast<int>(HcalOuter))) {
    if (ieta >  etaMax_[0])  ieta = etaMax_[0];
  } else if (det == static_cast<int>(HcalEndcap)) {
    if (ieta <= etaMin_[1]) ieta = etaMin_[1];
  }
  return std::make_pair(subdet,ieta);
}

std::pair<int,int> HcalNumberingFromPS::getPhi(const int& det, const int& ieta, 
  ...
  if (units < 1) units = 1;
  int iphi_skip = iphi;
  if      (units==2) iphi_skip  = (iphi-1)*2+1;
  else if (units==4) iphi_skip  = (iphi-1)*4-1;
